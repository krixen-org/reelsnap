...
export default function YouTubeDownloader() {
  ...
  const fetchVideo = async () => {
    ...
    try {
      const res = await fetch(getApiUrl("/youtube"), {
        method: "POST",
    ...
    <main className="min-h-screen px-4 py-8 sm:py-12">
      <div className="w-full max-w-4xl mx-auto animate-fadeIn">
        ...
        <header className="text-center mb-8 sm:mb-12">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-bold mb-3 text-gradient from-red-400 to-red-600">
            YouTube Downloader
          </h1>
        ...
        {/* Input Section */}
        <section className="glass-card rounded-2xl p-6 sm:p-8 mb-6 border-red-500/30">
          <div className="space-y-4">
            <div className="relative">
              <input
                type="text"
                className="w-full px-4 py-4 bg-white/10 border border-white/20 rounded-xl text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
            ...
            <button
              ...
              className="w-full bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 disabled:from-gray-600 disabled:to-gray-700 disabled:cursor-not-allowed text-white font-semibold py-4 px-6 rounded-xl transition-all duration-300 transform hover:scale-[1.02] active:scale-[0.98] shadow-lg shadow-red-500/30 disabled:shadow-none"
              aria-label="Fetch YouTube video"
            >
              {loading ? (
